...
Tree structures also appear directly in machine learning architectures. Hierarchical softmax~\citep{Morin2005} uses a binary tree to reduce softmax computation from $O(V)$ to $O(\log V)$, though the tree structure is fixed. \citet{Tanno2019} introduce Adaptive Neural Trees that grow tree architectures during training, the closest architectural precedent to the octonionic trie, though ANTs use backpropagation for growth decisions rather than algebraic signals.
