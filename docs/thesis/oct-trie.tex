...
A stronger bound holds when the elements are concentrated near a common \emph{element}. Crucially, this bound requires only alternativity --- the center element need not lie in any quaternionic subalgebra:

\begin{corollary}[Element proximity bound]
\label{cor:element-proximity}
Let $q \in S^7$ be any unit octonion, and let $a, b, c \in S^7$ satisfy $\lVert a - q \rVert, \lVert b - q \rVert, \lVert c - q \rVert \leq \epsilon$. Then:
\begin{equation}
...
\begin{proof}
Write $a = q + \delta a$, $b = q + \delta b$, $c = q + \delta c$ with $\lVert \delta a \rVert, \lVert \delta b \rVert, \lVert \delta c \rVert \leq \epsilon$. (Since $a, q \in S^7$, the perturbation satisfies $\langle q, \delta a \rangle = -\lVert \delta a \rVert^2 / 2 = O(\epsilon^2)$; this sphere constraint does not affect the order analysis below.) Expanding $[a,b,c]$ by trilinearity, the $O(1)$ term $[q,q,q] = 0$ by the alternating property. The three $O(\epsilon)$ terms are $[\delta a, q, q]$, $[q, \delta b, q]$, and $[q, q, \delta c]$, each of which vanishes: the first and third by alternativity ($[x, y, y] = [y, y, x] = 0$), and the second by flexibility ($[x, y, x] = 0$, which holds in every alternative algebra). No property of $q$ beyond membership in $\Oct$ is used. All surviving terms contain at least two perturbations, giving $O(\epsilon^2)$.
\end{proof}
...
\begin{itemize}[nosep]
  \item \textbf{Within-class suppression} (\cref{cor:element-proximity}): Clustered data has small associator norms, bounded quadratically by the cluster radius. This follows from \emph{alternativity alone} and holds for clusters around any point in $S^7$, regardless of subalgebra alignment. The trie's novelty detection is therefore more robust than a subalgebra-centric analysis would suggest.
  \item \textbf{Between-class separation} (\cref{sec:fano-geometry}): Elements drawn from classes assigned to distinct subalgebras produce associator norms bounded \emph{away} from zero, with the gap determined by the Fano plane geometry. This is where the subalgebra structure is load-bearing.
\end{itemize}

\noindent Two mechanisms --- quadratic floor suppression from alternativity, geometric ceiling elevation from the Fano plane --- emerge from the same algebraic object. This is the sense in which the octonionic trie achieves \emph{algebraic economy}: a single 8-dimensional non-associative algebra provides both noise suppression and class discrimination, through two independent properties of its multiplication structure.

\Cref{prop:proximity} provides the complementary (but weaker) guarantee that even elements spread broadly across a subalgebra, without clustering around a single point, have associator norms bounded linearly by their distance from the subalgebra.
