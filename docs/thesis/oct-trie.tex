...
\begin{table}[ht]
\centering
\caption{Comparison of the octonionic trie with existing memory-augmented architectures.}
...
The associator compatibility threshold, rumination consistency threshold, and depth error threshold are currently global constants. Whether these should be per-node adaptive values or derived from data distribution percentiles is an open question with significant impact on growth rate and sensitivity. This question is addressed theoretically in \cref{sec:adaptive-thresholds} and remains an active area of empirical investigation.

\subsection{Scalability and Capacity}

The trie's node count grows with data complexity (26,042 nodes for 60,000 MNIST samples). Whether the consolidation mechanisms (sibling absorption, child merging) can bound this growth in practice, and what the effective capacity limit is for a trie of bounded depth, are open questions with direct implications for deployment to larger-scale problems. Empirical characterization on datasets beyond MNIST --- including CIFAR-10, larger numbers of classes, and non-image modalities --- is needed to establish the practical operating regime.
...
\begin{equation}
  \lVert [a, b, c] \rVert = O(\epsilon) \quad \text{as } \epsilon \to 0.
  \label{eq:proximity-bound}
\end{equation}
More precisely, there exists a constant $K > 0$ (depending only on the structure constants of $\Oct$) such that $\lVert [a, b, c] \rVert \leq K \epsilon$ for all $\epsilon < 1$.
\end{proposition}
...
\end{align}
Every remaining term contains at least one perpendicular component of norm $O(\epsilon)$. Since the associator is trilinear and bounded by $\lVert [x,y,z] \rVert \leq 2\lVert x \rVert \lVert y \rVert \lVert z \rVert$, and the parallel projections have norm at most~$1$, each term is $O(\epsilon)$.

The bound is tight: for the subalgebra $\mathcal{S} = \text{span}_\Real\{1, e_1, e_2, e_4\}$, the associator $[e_1, e_2, e_3] = -2e_6$ has norm~$2$, so generic perturbations with a single $O(\epsilon)$ component perpendicular to $\mathcal{S}$ contribute at $O(\epsilon)$, not $O(\epsilon^2)$.
\end{proof}

A stronger bound holds when the elements are concentrated near a common \emph{element}, not merely near a common subalgebra:

\begin{corollary}[Element proximity bound]
\label{cor:element-proximity}
Let $q \in S^7$ be a unit octonion lying in a quaternionic subalgebra $\mathcal{S}_\ell$, and let $a, b, c \in S^7$ satisfy $\lVert a - q \rVert, \lVert b - q \rVert, \lVert c - q \rVert \leq \epsilon$. Then:
\begin{equation}
  \lVert [a, b, c] \rVert = O(\epsilon^2) \quad \text{as } \epsilon \to 0.
  \label{eq:element-proximity-bound}
\end{equation}
\end{corollary}

\begin{proof}
Write $a = q + \delta a$, $b = q + \delta b$, $c = q + \delta c$ with $\lVert \delta a \rVert, \lVert \delta b \rVert, \lVert \delta c \rVert \leq \epsilon$. Expanding $[a,b,c]$ by trilinearity, the $O(1)$ term $[q,q,q] = 0$ by the alternating property. The three $O(\epsilon)$ terms are $[\delta a, q, q]$, $[q, \delta b, q]$, and $[q, q, \delta c]$, each of which vanishes: the first and third by alternativity ($[x, y, y] = [y, y, x] = 0$), and the second by flexibility ($[x, y, x] = 0$, which holds in every alternative algebra). All surviving terms contain at least two perturbations, giving $O(\epsilon^2)$.
\end{proof}

\Cref{cor:element-proximity} is the operationally relevant bound for the trie. Within a well-functioning branch, the input $x$, child routing key $c$, and parent node $n$ are all close to their respective cluster centers. The within-class spread $\sigma$ plays the role of $\epsilon$, and the quadratic suppression $O(\sigma^2)$ explains why the associator threshold can cleanly separate within-class triples (small) from between-class triples (bounded away from zero).

\Cref{prop:proximity} provides the weaker but still useful guarantee that even elements spread across a subalgebra (not clustered around a single point) have associator norms bounded linearly by their distance from the subalgebra.
...
\textbf{Evidence for the conjecture.} The within-class bound follows from \cref{cor:element-proximity}: elements of class $k$ lie within distance $\sigma$ of the class mean $\mu_k$, which itself lies near $\mathcal{S}_{\ell(k)}$. Since within-class triples are concentrated around a common element, the associator norm is $O(\sigma^2)$ by the element proximity bound. (The weaker subalgebra bound of \cref{prop:proximity} gives only $O(\delta + \sigma)$; the quadratic rate requires element-level concentration.) The between-class lower bound follows from the fact that elements from classes assigned to different subalgebras span genuinely non-associative structure; the associator norm is bounded away from zero by the inter-subalgebra separation.
